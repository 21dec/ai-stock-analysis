...
3. **입력 JSON 작성** — 스킬 출력에서 다음 값을 추출해 임시 JSON 파일로 만든다
   ...
   ```json
   {
     ...
     "chart": {
       "labels": ["07-01", "07-08", "07-15", "07-22", "07-29", "08-05"],
       "price": [241.7, 243.62, 254.96, 244.85, 226.65, 277.42],
       "ma20": [null, null, null, 242.34, 243.31, 247.42]
     }
   }
   ```
   ...
   - `chart`: 가격+MA20 라인 차트용 실데이터. `labels`(날짜 라벨), `price`(종가,
     실제 시세 데이터에서 가져온 값 — 절대 임의로 만들어내지 않는다), `ma20`
     (20일 이동평균 롤링값, 계산 불가능한 초기 구간은 `null`)을 같은 길이의
     배열로 채운다. 세 배열의 길이는 반드시 같아야 한다.
